...
\begin{aligned}
\mathrm{min}J^{C}(f,g):=&k^{2}\E[f(X_0)-X_0]^{2}+\\
&\E[(f(X_0)-g(f(X_0)+N))^{2}]
\end{aligned}
\end{equation}
...
\begin{equation}\label{eq:minJw}
  \begin{aligned}
  \mathrm{min}J^{W}(f,g):=&k^{2}W_2(P,Q)^{2}+mmse(Q,\sigma^2)
  % &\E[(f(X_0)-g(f(X_0)+N))^{2}]
\end{aligned}
\end{equation}

where $W_2$ means Wasserstein-2 distance. Compared to the equation \eqref{minJw}, equation \eqref{eq:minJ} could be called as classic Witsenhausen cost.
...
From the classic view of Witsenhausen problem, 
...
\begin{table}[htbp]
  ...
  \hline
  \textbf{Optimizer}&\multicolumn{3}{|c|}{\textbf{J}} \\
  \cline{2-4} 
