...
\begin{equation}\label{eq:Jw}
  \min_{Q}J^{W}(f,g):=k^{2}W_2(P,Q)^{2}+mmse(Q,\sigma^2)
  \end{equation}

which is the same with
\begin{equation}
\min_{Q,g}=k^{2}W_2(P,Q)^{2}+mmse(Q,\sigma^2)
  % &\E[(f(X_0)-g(f(X_0)+N))^{2}]

\end{equation}

where $W_2$ means Wasserstein-2 distance. Compared to the equation \eqref{eq:Jw}, equation \eqref{eq:minJ} could be called as classic Witsenhausen cost.
